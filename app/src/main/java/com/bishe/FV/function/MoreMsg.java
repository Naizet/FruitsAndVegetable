...
import com.bishe.FV.dao.DataCar;
...
public class MoreMsg extends FragmentActivity{
	...
	public void MyOnClick(View v){
		if (v.getId()==R.id.xiang_put_in) {
			if (str) {
				Intent i = new Intent(MoreMsg.this,MainActivity.class);
				startActivity(i);
				getGoods();
				saveGoods();
				Toast.makeText(MoreMsg.this, "成功加入购物车", 1).show();
			}else {
				Log.v("TAG", "请登录");
				Intent i = new Intent(MoreMsg.this,Login.class);
				startActivity(i);
			}
		}else if (v.getId()==R.id.xiang_buy_now) {
			Toast.makeText(MoreMsg.this, "此服务未开放", 1).show();
		}
